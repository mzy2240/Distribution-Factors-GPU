...
% build Bx matrix
dTest_Bx = gpuArray(double(Bx'));
dTest_frombus = gpuArray(int32(frombus));
dTest_tobus = gpuArray(int32(tobus));
dTest_BranchStatus = gpuArray(int32(BranchStatus));
dTest_xline = gpuArray(double(xline));
BuildBx_Gridsize = ceil(size(Bx,1)/32);
BuildBx = parallel.gpu.CUDAKernel('BuildBx.ptx', 'BuildBx.cu', 'BuildBx');
BuildBx.GridSize = [BuildBx_Gridsize BuildBx_Gridsize 1];
BuildBx.ThreadBlockSize = [32 32 1];
dTest_Bx = feval(BuildBx, dTest_Bx, dTest_frombus, dTest_tobus, dTest_BranchStatus, dTest_xline , numline , size(dTest_Bx,1));
B = gather(dTest_Bx);
